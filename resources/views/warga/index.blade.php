                        ...
                        <tbody>
                            @foreach ($wargas as $warga)
                                <tr>
                                    ...
                                    <td>
                                        @if ($warga->status_keluarga == '0')
                                            <span
                                                class="btn btn-success btn-sm">{{ ucfirst($warga->status_keluarga) }}</span>
                                        @elseif ($warga->status_keluarga == '1')
                                            <span
                                                class="btn btn-danger btn-sm">{{ ucfirst($warga->status_keluarga) }}</span>
                                        @endif
                                    </td>
                                    {{-- <td>
                                <!-- Edit Data Kader -->
                                <a href="{{ route('wargas.update', $warga->id) }}" class="btn btn-warning btn-sm mr-2 my-1 edit-btn" data-toggle="modal" data-target="#editUserModal" data-name="{{ $user->name }}" data-nik="{{ $user->nik }}" data-username="{{ $user->username }}">
                                ...
                                <!-- Ubah Status Kader -->
                                @if ($warga->status_keluarga == '0')
                                <form action="{{ route('wargas.status.update', $warga->id) }}" method="POST" class="d-inline">
                                    @csrf
                                    @method('PATCH')
